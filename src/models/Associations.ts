...
// Define types for roles and associations
interface RoleStatus {
  ...
  correspondingProviderRole?: Role;
  correspondingUserRole?: Role;
}

interface Role {
  getStatus: () => RoleStatus;
  transfer: (expressor: SocialNode, newHolder: SocialNode) => string;
  toggleDesiring: (expressor: SocialNode) => string;
  togglePlaying: (expressor: SocialNode) => string;
  getHolder: () => SocialNode;
}
...
export class SocialNode extends Node {
  ...
  toggleDesiring(role: Role): string {
  ...
  togglePlaying(role: Role): string {
    // Verify we are the holder of this role
    if (role.getHolder() !== this) {
  ...
  distributeSurplus(): void {
    ...
    surplus.forEach(role => {
      ...
      const item = surplusByTypeAndRole.get(key);
      if (item) {
    ...
    const distribution = this.mutualFulfillmentDistribution;
...
export class Association {
  ...
  compositions: Association[];

  constructor(name: string, totalShares: number, initialHolder: SocialNode, composes: Association[] = []) {
    this.name = name;
    this.associations = [];
  ...
  socialRelation(userRoleHolder: SocialNode, providerRoleHolder: SocialNode): AssociationPair {
    // State variables for the association
    let currentUserRoleHolder: SocialNode = userRoleHolder;
    let currentProviderRoleHolder: SocialNode = providerRoleHolder;
    let playable = true;
    ...
    const userRole: Role = {
      ...
      getHolder: (): SocialNode => currentUserRoleHolder
    };
    ...
    const providerRole: Role = {
      ...
      getHolder: (): SocialNode => currentProviderRoleHolder
    };
